...
void USBDsetDeviceAddress(set_address_t token, uint8_t address) {
  if (token == SET_ADDRESS_RESET || token == SET_ADDRESS_REQUEST) {
    ...
    dcfg.d32 = FS_USB_OTG_DREGS->DCFG;
    dcfg.b.dad = address;
...
void USBDcontinueInTransfer(uint8_t ep) {
  ...
  if (txBufferCount[ep] == 0) {
    FS_USB_OTG_DREGS->DIEPEMPMSK &= ~(1 << ep);
    pTxBuffer[ep] = 0;
  }
  else {
    FS_USB_OTG_DREGS->DIEPEMPMSK |= 1 << ep;
...
void USBDdisableAllNonControlEndPoints() {
  ...
  daintmsk.d32 = FS_USB_OTG_DREGS->DAINTMSK;
  daintmsk.b.oepm &= 1;
  daintmsk.b.iepm &= 1;
  FS_USB_OTG_DREGS->DAINTMSK = daintmsk.d32;
  FS_USB_OTG_DREGS->DIEPEMPMSK &= 1;
  ...
  for (i = 1; i < EP_MAX_COUNT; ++i) {
    ...
    FS_USB_OTG_DINEPS[i].DIEPCTLx = depctl.d32;
...
usb_result_t USBDendPointConfigureEx(uint8_t endPoint,
  ...
  if (type == CONTROL_TRANSFER && endPoint == 0 &&
    ...
    /* Configure RX FIFO. */
    fifoTop = RX_FIFO_DWORDS_SIZE;
    FS_USB_OTG_GREGS->GRXFSIZ = fifoTop;
    ...
    FS_USB_OTG_GREGS->DIEPTXF0 = txfifosize.d32;

    /* Configure endpoint registers. */
    deptsiz.d32 = 0;
    deptsiz.b.xfrsiz = rxBuffSize;
    deptsiz.b.pktcnt = 1;
    FS_USB_OTG_DINEPS[0].DIEPTSIZx = deptsiz.d32;
    deptsiz.d32 = 0;
    deptsiz.b.xfrsiz = rxBuffSize;
    deptsiz.b.pktcnt = 1;
    deptsiz.b.stupcnt_mcnt_rxpid = 1; /* only one back-to-back setup */
    FS_USB_OTG_DOUTEPS[0].DOEPTSIZx = deptsiz.d32;
    depctl.d32 = 0;
    switch (txBuffSize) {
      case 8:
    ...
    if (rxBuffSize > 0) {
      ...
      deptsiz.b.xfrsiz = rxBuffSize;
      deptsiz.b.pktcnt = 1;
      FS_USB_OTG_DOUTEPS[endPoint].DOEPTSIZx = deptsiz.d32;
      depctl.d32 = 0;
      ...
      depctl.b.epena = 1;
      FS_USB_OTG_DOUTEPS[endPoint].DOEPCTLx = depctl.d32;
    ...
    if (txBuffSize > 0) {
      ...
      txfifosize.b.depth = (txFifoSize + 3) >> 2;
      fifoTop += txfifosize.b.depth;
      FS_USB_OTG_GREGS->DIEPTXFx[endPoint - 1] = txfifosize.d32;
      ...
      FS_USB_OTG_DINEPS[endPoint].DIEPTSIZx = deptsiz.d32;
      depctl.d32 = 0;
      ...
      depctl.b.sd0pid_sevnfrm = 1;
      depctl.b.epena = 1;
      FS_USB_OTG_DINEPS[endPoint].DIEPCTLx = depctl.d32;
...
uint16_t USBDwrite(uint8_t ep, uint8_t const *buff, uint16_t count) {
  ...
  diepctl.d32 = FS_USB_OTG_DINEPS[ep].DIEPCTLx;
  ...
  dieptsiz.d32 = FS_USB_OTG_DINEPS[ep].DIEPTSIZx;
  dieptsiz.b.xfrsiz = count;
  dieptsiz.b.pktcnt = 1;
  dieptsiz.b.stupcnt_mcnt_rxpid = 1; /* for isochronous only */
  FS_USB_OTG_DINEPS[ep].DIEPTSIZx = dieptsiz.d32;
  ...
  FS_USB_OTG_DINEPS[ep].DIEPCTLx = diepctl.d32;
...
uint16_t USBDread(uint8_t ep, uint8_t *buff, uint16_t count) {
  ...
  /* Reenable endpoint. */
  doeptsiz.d32 = FS_USB_OTG_DOUTEPS[ep].DOEPTSIZx;
  doepctl.d32 = FS_USB_OTG_DOUTEPS[ep].DOEPCTLx;
  doeptsiz.b.xfrsiz = doepctl.b.mpsiz;
  doeptsiz.b.pktcnt = 1;
  doepctl.b.cnak = 1;
  FS_USB_OTG_DOUTEPS[ep].DOEPTSIZx = doeptsiz.d32;
  FS_USB_OTG_DOUTEPS[ep].DOEPCTLx = doepctl.d32;
...
usb_result_t USBDgetEndPointStatus(uint8_t epaddr, uint16_t *status) {
  ...
  if ((epaddr & ENDP_DIRECTION_MASK) == ENDP_IN)
    depctl.d32 = FS_USB_OTG_DINEPS[ep].DIEPCTLx;
  else
    depctl.d32 = FS_USB_OTG_DOUTEPS[ep].DOEPCTLx;
...
usb_result_t USBDsetEndPointHalt(uint8_t epaddr) {
  ...
  depctl.b.stall = 1;
  if ((epaddr & ENDP_DIRECTION_MASK) == ENDP_IN)
    FS_USB_OTG_DINEPS[ep].DIEPCTLx = depctl.d32;
  else
    FS_USB_OTG_DOUTEPS[ep].DOEPCTLx = depctl.d32;
...
uint16_t USBDwrite0(uint8_t const *buffer, uint16_t count) {
  ...
  txFifoSize.d32 = FS_USB_OTG_GREGS->DIEPTXF0;
  if (count > (txFifoSize.b.depth << 2))
    count = txFifoSize.b.depth << 2;
  pTxBuffer[0] = buffer;
  txBufferCount[0] = count;

  dieptsiz.d32 = FS_USB_OTG_DINEPS[0].DIEPTSIZx;
  dieptsiz.b.xfrsiz = count;
  dieptsiz.b.pktcnt = 1;
  FS_USB_OTG_DINEPS[0].DIEPTSIZx = dieptsiz.d32;
...
uint16_t USBDread0(uint8_t *buffer, uint16_t count) {
  ...
  /* Reenable endpoint. */
  doeptsiz.d32 = FS_USB_OTG_DOUTEPS[0].DOEPTSIZx;
  doeptsiz.b.xfrsiz = rxBuffSize0;
  doeptsiz.b.pktcnt = 1;
  FS_USB_OTG_DOUTEPS[0].DOEPTSIZx = doeptsiz.d32;
...
void USBDendPoint0TxVALID(void) {
  ...
  depctl.d32 = FS_USB_OTG_DINEPS[0].DIEPCTLx;
  depctl.b.cnak = 1;
  depctl.b.epena = 1;
  FS_USB_OTG_DINEPS[0].DIEPCTLx = depctl.d32;
}
...
void USBDendPoint0RxVALID(void) {
  ...
  depctl.d32 = FS_USB_OTG_DOUTEPS[0].DOEPCTLx;
  depctl.b.cnak = 1;
  depctl.b.epena = 1;
  FS_USB_OTG_DOUTEPS[0].DOEPCTLx = depctl.d32;
}
